  ...
  const handleEditAdoptionHistory = async (e) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        const currentAnimal = animals.data.filter(
          (animal) => animal.animalName == currentAnimalName,
        );
        console.log({
          ...adoptionHistory,
          photo: base64String,
        ...
        if (currentAnimal.length != 0) {
          const res = await updateAdoptionHistory(id, {
            ...adoptionHistory,
            photo: base64String,
            animalID: currentAnimal[0].id,
          });
          ...
          <form onSubmit={handleEditAdoptionHistory}>
            {Object.keys(adoptionHistory).length > 0 ? (
              <>
                <InputAnimalsPicture
                  name="photo"
                  initialImage={adoptionHistory.photo}
                />
                <FormInput
                  value={adoptionHistory.animalOwnerName || ''}
                  onChange={handleInputChange}
                  placeholder="Tutor"
                  name="animalOwnerName"
                />
                <FormInput
                  value={adoptionHistory.animalID || ''}
                  onChange={handleInputChange}
                  placeholder="Animal"
                  name="animalID"
                />
                <TextArea
                  value={adoptionHistory.adoptionReport || ''}
                  onChange={handleInputChange}
                  placeholder="Relato"
                  name="adoptionReport"
                />
