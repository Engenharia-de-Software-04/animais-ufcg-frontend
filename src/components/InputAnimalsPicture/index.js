...
export default function InputAnimalsPicture({ name, initialImage }) {
  ...
  return (
    <div className="input-animals-wrapper">
      <img
        ref={imgRef}
        className="img-input-animals-main img-input-animals"
        src={imageSrc}
        alt="imagem de gato e cachorro dormindo abraçados"
      />
      <div
        ref={inputWrapperRef}
        className="input-file-wrapper-main input-file-wrapper"
      >
        <input
          name={name}
      ...
      <p
        ref={textImageRef}
        className="input-animals-text input-animals-text-main"
        dangerouslySetInnerHTML={{ __html: textImage }}
      ></p>
    </div>
  );
}
